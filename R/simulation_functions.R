...
mixedpower <- function(model, data, fixed_effects, simvar,
  ...
  if (SESOI != F){
    ...
    SESOI_power_values["mode"] <- "rnorm"
    SESOI_power_values["effect"] <- row.names(rnorm_power_values)
...
simulateModel <- function(model, data, n_want, simvar, fixed_effects,  nsim = 100){
